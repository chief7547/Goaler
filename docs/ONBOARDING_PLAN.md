...
- **해금 조건:** 퀘스트 완료 횟수 3회 이상 → Stage 0.5 자동 해금(전리품 소개)
...
- **해금 조건:** 전리품 기록(loot_type이 있는 로그) 3회 이상 → Stage 1 자동 해금(에너지 안내)
...
- **해금 조건:** 에너지 상태 입력(energy_status가 있는 로그) 5회 이상 → Stage 1.5 해금(보스전 미리보기)
...
- **해금 조건:** 위 에너지 기준을 만족하거나, 보스전이 처음 정의되면 자동으로 Stage 1.5로 승급
