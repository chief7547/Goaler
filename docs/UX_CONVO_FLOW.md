...
1. **사용자 입력**
   - 예: "B로 할게"
2. **챗봇 확인**
   - "좋아요! 오늘은 B를 진행해 봅시다. 끝나고 나서 오늘 얻은 일일 기록만 한 줄로 공유해 주세요."
3. **상태 업데이트**
   - `choose_quest` 도구 호출 → `quest_id` 확정

## 3. 완료/피드백 수집
1. **챗봇 질문 (완료 시)**
   - 가장 초기 단계: "수고하셨어요! 오늘 계획은 여기까지예요." (일일 기록 소개 없음)
   - 이후 단계: "다이어리에 어떤 기록을 남기고 싶나요? [성과/깨달음/느낌] 중 하나를 골라주세요."  
     사용자가 칩을 고르면: "좋아요! 한 줄로만 남겨 주시면 기록에 추가할게요." (텍스트는 선택)
2. **컨디션 선택(선택 사항)**
   - 진행 단계 1 이후: "다음 턴을 위해 컨디션을 확인해 볼까요? 준비됨 / 페이스 유지 / 충전 필요 중 골라 주세요."
3. **도구 호출**
   - `log_quest_outcome`에 outcome, energy_status, loot_type, mood_note(일일 기록) 전달
4. **경고 레벨 안내**
   - 주의: "구멍을 메우는 시간을 가질까요? 내일은 컨디션을 살피는 변주를 넣어둘게요."
   - 경고: "연속으로 힘들다고 느끼셨어요. 오늘은 보완 퀘스트를 기본으로 준비할게요."
   - 위험: "충전 루틴이 필요해요! 이틀 동안 휴식 루틴을 따라가면 기록이 잘 보존될 거예요."
