...
- 목표 생성 단계:
  1. 사용자가 장기 목표를 설명하면 챗봇이 3~5개의 “핵심 퀘스트” 후보(현실 과업)를 제안하고, 사용자가 선택/수정한다. 초기 단계에서는 “핵심 마일스톤”이라는 표현을 먼저 사용한다.
  2. 선택된 핵심 퀘스트마다 “언제 달성하고 싶은가?”, “성공 기준은 무엇인가?”를 확인해 저장한다.
  3. 챗봇이 첫 번째 핵심 퀘스트를 기반으로 주간 체크포인트 2~3개를 제안하고, 사용자가 필요 시 수정한다. 초기 단계에서는 1~2개의 쉬운 준비 단계만 노출한다.
  4. 각 주간 단계에 대해 “이번 주 어떤 행동을 하면 좋을까요?”를 물어 일일/대안 퀘스트를 생성한다.
  5. 숫자형 지표가 필요한 경우 해당 핵심 퀘스트에 연결된 지표를 추가 선택하게 하고, 기본 흐름은 일일 퀘스트/일일 기록 중심으로 유지한다.
  6. 챗봇이 최종 요약(핵심 퀘스트 리스트 → 주간 단계 → 일일/대안 퀘스트)을 보여주고 사용자가 확정한다.
- 퀘스트 완료 흐름: 대시보드 또는 챗봇에서 “오늘의 추천 행동” 카드를 탭하거나 “완료했어”라고 말하면 완료 처리된다. 초기 진행 단계에서는 칭찬 메시지와 다음 일일 퀘스트 안내만 노출한다. 일일 기록 기능이 해금되면 칩(성과·깨달음·느낌)을 탭하는 것만으로 기록할 수 있고, 텍스트 입력은 선택 사항이다. 컨디션 버튼(준비됨/페이스 유지/충전 필요)은 충전 루틴 개념이 소개된 이후에만 표시된다.
...
| --- | --- | --- | --- |
| 대시보드 | 오늘 해야 할 행동과 현재 집중 목표를 한눈에 보여주는 허브 | “오늘의 추천 행동” 히어로 카드, 집중 목표 카드(남은 기간·동기 요약), 오늘/주간 퀘스트 체크리스트(남은 횟수를 크게 표시), 진행 단계/XP 표시, 챗봇 대화 진입 버튼 | MVP: 단일 목표 집중 / 확장: 다중 목표 카드 리스트 |
| 목표 상세 | 목표의 전체 맥락과 퀘스트 상태를 확인하고 관리하는 화면 | 핵심 정보 카드(SDT 메시지 + 프로필 기반 톤), 퀘스트 보드(완료/보류/건너뛰기 토글 + 칭찬/리마인더 + 진행 단계별 챕터 퀘스트), 오늘의 추천 행동 히스토리, 최근 대화 요약, 리마인더 설정 미리보기 | MVP 완료 |
...
| 변주 선택 모달 (챗봇/대시보드 통합) | LLM이 제시한 변주 2~3개를 비교하고 선택 | 퀘스트 카드(난이도 배지, 예상 시간, 변주 이유), 일일 기록 칩(성과/깨달음/느낌) 선택 + 한 줄 입력(선택), 컨디션 버튼(준비됨/페이스 유지/충전 필요), “오늘은 건너뛸래요” 버튼 | Progressive Unlock |
...
- 진행 단계 승급/경고: `player_progress` 변경 시 대시보드 상단에 텍스트 연출(예: “Spark Awakening → Momentum Forge”)을 노출하고, 승급 이유/경고 단계와 손실 방지 팁을 안내한다. 경고 임박 시 경고 배너와 충전/보완 제안을 함께 제공한다.
- 경고 레벨 UI: 컨디션 버튼에서 `NEEDS_POTION`이 누적되면 경고 배너(주의/경고/위험)를 표시하고, 충전 루틴/보완 퀘스트 버튼을 추천한다. 위험 단계에서는 “충전 루틴” 카드(이틀치 회복 루틴 + 기록 보호 설명)를 자동으로 띄운다.
...
- 긍정 경험 강조: 매일 “일일 기록”을 먼저 노출시키고, 휴식 필요 여부는 보조 질문으로 처리.
