...
## 1. 대시보드 상단 카드 (우선순위 1)
- 구성
- 현재 진행 단계 명칭 + 텍스트 연출 (예: “Momentum Forge · 핵심 퀘스트: 사업자등록 완료”)
...
- 텍스트 모형
  ```text
  ┌───────────────────────────────────────────────────────────────┐
  │ 진행 단계: Momentum Forge  |  핵심 퀘스트: 사업자등록 완료     │
  │ Progress: ▓▓▓░░ (3/5 단계 완료)                               │
  ...
  ```
- 노출 조건
- 가장 초기 단계: 진행도/경고/충전 버튼 숨김, 간단한 퀘스트 카드만 노출
- 이후 단계: 진행도 막대와 일일 기록 강조 문구 활성화
...
- 노출 조건
- 가장 초기 단계: 전체 패널 비표시
- 진행 단계 1.5: “핵심 마일스톤 미리보기”로 축약, 이후 전체 패널 활성화
  - 테마별 아이콘/라벨 교체 (게임: 보스 아이콘, 전문가: 체크마크)

## 3. 다이어리 (우선순위 2)
- 탭 구성: 일일 Memory Shard / 주간 Combo Gem / Milestone Relic
- 각 Shard 카드: 기록 칩 아이콘(성과/깨달음/느낌) + 한 줄 텍스트 + 날짜 + 핵심 퀘스트 태그
...
- 텍스트 모형
  ...
  │ “사업자등록 서류 정리 완료” · 핵심 퀘스트: 등록 │
  └──────────────────────────────────────────────┘
  공유하기 ▷
  ```
- 노출 조건: 일정 단계 이상부터 메뉴에 등장, 이전 단계에서는 숨김

## 4. 회고 화면 핵심 퀘스트 섹션 (우선순위 2)
- “Core Quest Log” 카드
  - 이번 주 목표 핵심 퀘스트 이름, 준비 단계 완료율, 일일 기록 요약
  - “다음 주 기록 예상” 텍스트 (LLM이 제안)
- 복귀/휴식 버튼: 다음 주 전략(공격/수비/회복) 선택
- 텍스트 모형
  ```text
  ┌────────── Core Quest Log ─────────┐
  │ 핵심 퀘스트: 사업자등록 완료 (3/4 준비 단계 완료) │
  │ 이번 주 기록: “홈택스 계정 생성”, “서류 검토” │
  │ 다음 주 추천: “신청서 제출용 실습”               │
  │ 전략 선택: [공격 지속] [수비(보완)] [회복]       │
  └────────────────────────────────────┘
  ```
- 노출 조건: 진행 단계 1 이후 회고 화면에 활성화, 전문가 모드에서는 “성과 리뷰”라는 라벨 사용

## 5. 상호작용 흐름 정리
- 진행 단계 변경 → 대시보드 상단 카드가 업데이트되고, 해당 단계의 핵심 퀘스트가 강조 (테마에 따라 용어 변경)
- 핵심 퀘스트 완료 → 타임라인 카드에 완료 배지 + Relic 아이콘 표시, 회고 화면에서 하이라이트
- 핵심 퀘스트 미완료 → 타임라인 카드가 “재도전 준비” 단계로 돌아가고, 충전 퀘스트 CTA 노출
- 초기 단계에서는 일일 기록·핵심 퀘스트 UI가 숨겨져 사용자에게 최소 루프만 노출되도록 가드
